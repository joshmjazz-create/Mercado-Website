...
export default function Music() {
  ...
  useEffect(() => {
    const fetchMusicData = async () => {
      try {
        ...
        const UPCOMING_FOLDER_ID = "1QLjaPQHjqguX1bD4UDVyN2xaPyCvvLN6"; // existing subfolder structure
  ...
  // Fetch upcoming albums from folder structure
          ...
          <DialogContent className="bg-white border-purple-800">
            ...
            <div className="grid grid-cols-1 gap-4">
              {selectedAlbum?.links.spotify && (
                <Button onClick={() => handlePlatformClick(selectedAlbum.links.spotify)} className="w-full bg-green-600 hover:bg-green-700 text-white p-4 h-auto flex items-center justify-center">
                  <FaSpotify className="mr-3 text-xl" /> Listen on Spotify <ExternalLink className="ml-auto w-4 h-4" />
                </Button>
              )}
              {selectedAlbum?.links.applemusic && (
                <Button onClick={() => handlePlatformClick(selectedAlbum.links.applemusic)} className="w-full bg-gray-800 hover:bg-gray-900 text-white p-4 h-auto flex items-center justify-center">
                  <FaApple className="mr-3 text-xl" /> Listen on Apple Music <ExternalLink className="ml-auto w-4 h-4" />
                </Button>
              )}
              {selectedAlbum?.links.youtube && (
                <Button onClick={() => handlePlatformClick(selectedAlbum.links.youtube)} className="w-full bg-red-600 hover:bg-red-700 text-white p-4 h-auto flex items-center justify-center">
                  <FaYoutube className="mr-3 text-xl" /> Watch on YouTube <ExternalLink className="ml-auto w-4 h-4" />
                </Button>
              )}
            </div>
          </DialogContent>
