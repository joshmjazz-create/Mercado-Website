  ...
  const fetchUpcomingAlbums = async (folderId: string): Promise<Album[]> => {
    try {
      ...
      for (const subfolder of data.files) {
        ...
        let infoDoc = files.find((f: any) => f.mimeType === "application/vnd.google-apps.document");

        let artist = "Unknown Artist";
        let title = subfolder.name;

        if (infoDoc) {
          try {

            const docRes = await fetch(
              `https://docs.google.com/document/d/${infoDoc.id}/export?format=txt`
            );
            ...
            console.error("Failed to read info doc:", err);
          }
        }
        ...
        upcomingAlbums.push({
          ...
          audioPreviewUrl: audioFile
            ? `https://www.googleapis.com/drive/v3/files/${audioFile.id}?alt=media&key=${API_KEY}`
            : "",
          links: {},
        });
  ...
  const renderSection = (title: string, sectionAlbums: Album[]) => {
    ...
    return (
      ...
      <div className="opacity-0 translate-y-4 animate-in mb-12">
        ...
        <h2 className="text-2xl font-semibold text-gray-700 mb-8 text-left underline decoration-gray-700">
          {title}
        </h2>
